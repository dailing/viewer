...
- `internal/acp/` implements the bounded NDJSON JSON-RPC ACP stdio client used by Go chat, including initialize/new/load/prompt/cancel, response-id correlation, session/update dispatch, stderr diagnostics, malformed-line tolerance, and process-exit failure propagation. `internal/plugins/chat/` implements the M6b Hermes Super Workspace plugin: workspace/Role/routing configuration is read and written through config-store under `plugins.viewer-chat`, chats/messages/turns/chat×Role session triples live in GORM/modernc `chat.sqlite3`, explicit or one-shot OpenAI-compatible LLM routing feeds a strictly ordered multi-Role relay, and message/completion events carry structured sender identity. It keeps the M6a `send-message`/`stop` slot names as aliases while the primary submission contract is `chat:_:dispatch`.
...
- `scripts/mock_acp_agent.py` is a deterministic, no-LLM ACP stdio peer for initialize/new/load/prompt/update/cancel. `scripts/smoke_chat.py` launches isolated config-store + chat plugins, creates two Roles and one chat, activates its retained mailbox, verifies the explicit dual-Role relay/event sender order and chat×Role DB rows, and checks the unconfigured-router error; `smoke_all.sh` includes it as the ninth suite.
...
- M6b chat frontend module registered through the same in-repo loader. `ChatPane.vue` renders a per-chat interleaved timeline without automatic scrolling, explicit/automatic dispatch controls, active-turn state, stop, and links to the management panes. `ChatsPanel.vue` owns chat CRUD/pinning/activation, `RolesPanel.vue` owns the Hermes-only Role form, and `RoutesPanel.vue` provides validated JSON routing-policy editing. Shared bus payload mirrors live in `types.ts`.
