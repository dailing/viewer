...
- `/api/config` GET/PUT: reads and writes nav appearance, workspace count and workspace heat timing, Codex model options, voice model/language/translation options, Task DAG API base URL, Markdown theme config, user profiles, and default user in `~/.view/config.json`. Normal settings saves preserve existing user profiles unless the JSON payload explicitly changes them. Sidebar current directory, pinned paths, and per-workspace open-order visit timestamps live in per-user workspace files under `~/.view/users/{user}/workspaces.json`; traditional workspace agent associations live in DB conventional roles.
- `/api/workspaces` routes: traditional pane workspace state plus Super Workspace-style workspace/role APIs. GET `/api/workspaces` returns active workspace id, config count, DB-derived workspace summaries, and per-slot pane snapshots; GET `/api/workspaces/workspaces` returns only active workspace id plus summaries; POST `/api/workspaces/active-workspace/{workspace_id}` switches the active traditional workspace; GET `/api/workspaces/role-statuses/{workspace_id}` returns simplified role statuses backed by `super_workspace_driver_runs`; POST `/api/workspaces/{id}/roles` and DELETE `/api/workspaces/{id}/roles/{session_ref}` attach/remove conventional role associations for the `user` query profile; `/api/workspaces/config` GET/PUT still manages count and heat config; PUT `/api/workspaces/{id}` writes pane/sidebar snapshot state. Workspace count is stored globally in `~/.view/config.json`; layout, sidebar, pinned file paths, and visit heat remain in per-user `~/.view/users/{user}/workspaces.json`. Traditional workspace agent session association is sourced from DB roles in `~/.view/agent-history.sqlite3`: workspace slot `1` maps to DB workspace id `{user}:workspace:1`, slot `2` to `{user}:workspace:2`, and so on, with one stable role row per associated `provider:session_id`. `workspaces[]` and `agent_session_ids` are derived response data repopulated from DB on read and are not persisted to `workspaces.json`.
...
- `read_config()` / `write_config(config)`: load and save nav appearance, workspace count/heat timing, Codex model options, Task DAG API base URL, Markdown theme config, user profiles, and default user. Missing config defaults to five numbered traditional workspaces; old workspace-state keys in config are ignored rather than migrated.
- `read_workspaces(user_id)` / `write_workspace(..., user_id)` / `set_active_workspace(..., user_id)` / `write_workspace_config(..., user_id)`: load and save `~/.view/users/{user}/workspaces.json` state plus `~/.view/config.json` workspace count. `workspaces.json` stores active workspace id and per-slot layout JSON, active pane id, current sidebar directory, pinned file paths, per-workspace visit timestamps, and update timestamp. Traditional workspace summaries and agent refs are not stored in this JSON; `read_workspaces()` repopulates response-only `workspaces[]` and `agent_session_ids` from conventional DB workspace/role rows.
...
- `WorkspaceConfig.count` defaults to 5 and controls how many numbered traditional workspace rows are ensured in the DB and shown in the sidebar activity rail. `heat_interval_seconds` and `heat_step_percent` control frontend workspace button heat: on each interval the active workspace score moves toward 1 by the configured percentage and inactive scores move toward 0 by the same percentage. `WorkspaceData.count` mirrors the count config in `/api/workspaces` responses for frontend convenience, `WorkspaceData.workspaces` mirrors the Super Workspace summary-list shape, and persisted workspace slots in per-user `~/.view/users/{user}/workspaces.json` include per-workspace visit timestamps used to sort the current folder by most recent visit.
...
- Exposes workspace list/activation helpers and role status summaries. Role status is deliberately simplified for the frontend role rail: `queued`, `claimed`, and `running` driver-run rows map to `busy`; when no busy row exists, only the latest driver-run row can make the role `failed`; all other cases are `idle`.
...
- Defines `SuperWorkspaceMessageCreate` for new user/role-originated query messages. The payload carries optional `parent_message_id` and `sender_role_id` lineage fields so a later role-to-role request can be tied back to the message that produced it.
- `SuperWorkspaceRuntime.submit()` parses leading `@Role ` query prefixes, persists the query message without the dispatch prefix, auto-dispatches through `SuperWorkspaceManager.dispatch()` when no explicit mentions are supplied, marks the run queued, expands selected role names to all concrete role ids with that display name, and writes one queued `super_workspace_driver_runs` dispatch-task row per concrete role id. Role mention keys are derived from role names using ASCII variable-name characters so frontend insertion and backend parsing agree.
...
- Root shell: top bar, sidebar drawer/pinned layout, legacy pane workspace, full-page settings, full-page loop tasks, full-page Agent Task DAG, and full-page Super Workspace.
- Defaults to the full-page Super Workspace as the primary app window. The top bar includes an explicit Workspace button for entering the older pane workspace as an auxiliary page.
...
- Agent provider/session list is loaded on startup and polled every 15 seconds through `stores/agents.ts`. Workspaces receive a derived `workspaces[]` summary list and `agent_session_ids` list from DB conventional workspace/role rows; new or opened sessions are attached as roles in the active workspace so the Agents sidebar is workspace-scoped rather than directory-scoped. Removing a row from the sidebar removes the conventional DB role association and matching panes; it does not delete viewer session metadata or canonical provider history.
...
- Loads per-user Super Workspace workspace state through `/api/super-workspace/workspaces`, loads active workspace data through `/api/super-workspace`, edits common prompt plus role name/provider/cwd/model/fixed rules, and stores the current backing agent session ref on each role.
- Roles live in a narrow left rail similar to workspace buttons. Clicking an unselected role only highlights it as the direct dispatch target; clicking the already selected role opens a right-side overlay settings panel. The add-role button sits at the bottom of the rail, while renewing a role session is handled inside the role settings panel.
- The main chat area keeps the history as the scrollable region and pins the composer at the bottom in a chat-style input card. The `/runs` API and local `runs` state stay newest-first for pagination; the page reverses only for rendering so older messages appear above newer ones and the newest message is nearest the bottom composer. Loading older history appends older rows to the newest-first state, which renders them above the current list.
...
- `Renew` creates a fresh provider session using the current common prompt plus the same fixed role rules, then patches the role's `session_ref` so future dispatches resume the new context.
- The composer creates a persisted run through `/api/super-workspace/runs`. Clicking a role inserts its leading `@Role ` mention into the composer if it is not already present; removing that text is how the user removes an explicit dispatch target. Visible Super Workspace user messages and final role responses have small cite buttons that insert `@msg-{message_id} ` into the same leading prefix. Mentions dispatch by role name, so duplicate-name roles are all selected. Citation tokens are not separate request fields; the frontend sends only the composer text, and backend `super_workspace_runtime.py` parses leading role/citation tokens, writes citation edges and queued dispatch-task rows, and leaves execution to the independent Super Workspace worker process.
...
- Uses `/api/super-workspace/role-statuses/{workspace_id}` as the source for role rail status chips, while display history comes from the agent history DB through lazy `/api/super-workspace/runs` flat feed pages. Focus snapshots from the shared agent API are still used for context percentage and link base directories, not as the primary role lifecycle source. SSE-triggered incremental fetches update the loaded item window and refresh role statuses when the backend reports changed runs; the frontend does not request provider resyncs. The page renders flat display items directly: user query items show dispatch state and target chips, assistant `message:assistant` items show the target role label and Markdown-rendered message text, while reasoning/tool/thinking rows stay hidden at the display-feed query layer.
...
- Refreshes Git status on mount, when the Files directory changes, and every 15 seconds while mounted; overlapping refreshes are skipped.
...
- Pinia store for the active user's `~/.view/users/{user}/workspaces.json` state plus the config-derived workspace count and DB-derived workspace summaries returned by `/api/workspaces`.
- Actions: `load()`, `snapshotFor()`, `saveSlot()`, and `activate()` call `/api/workspaces` endpoints and track the active numbered workspace. Activation uses the Super Workspace-style `/api/workspaces/active-workspace/{workspace_id}` route.
- Tracks active workspace agent refs through `activeAgentSessionRefs`; refs are response data from DB role associations attached/removed through `/api/workspaces/{id}/roles`, not persisted workspace JSON fields.
...
- `~/.view/users/{user}/workspaces.json`: active workspace id plus per-slot split tree, active pane, open file paths, open terminal IDs, sidebar directory, pinned files/folders, per-workspace visit timestamps/open ordering, and update timestamp, managed by `/api/workspaces?user=...`. Traditional workspace summaries and agent associations are persisted as conventional DB workspace/role rows and returned as derived `workspaces[]` / `agent_session_ids`, not stored in this JSON.
