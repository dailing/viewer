...
- Per-user Super Workspace role manager backed by `super_workspaces` and `super_workspace_roles` rows in `~/.view/agent-history.sqlite3`. The current default workspace is named `Default Super Workspace` and uses user-scoped id `{user}:default`.
