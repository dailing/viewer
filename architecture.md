...
- `/api/super-workspace` routes: per-user Super Workspace common prompt, role storage, persisted message/query history, and LLM dispatch. Role definitions live in `~/.view/users/{user}/super-workspace.json` and contain stable role id, non-unique display name, fixed rules/description, provider, cwd, model, and the current backing agent session ref; the same file stores a `common_prompt` used when a role session is first created. `/api/super-workspace/runs` list-paginates Super Workspace messages whose `query` field is non-empty, backed by `~/.view/agent-history.sqlite3`; the list route is a DB read and does not touch provider rollout/state files. It also accepts `after=<updated_at>` for SSE-driven incremental reads, returning only runs whose query row or dispatch task changed after the cursor plus `next_after` for the next request. POST `/api/super-workspace/runs` and `/api/super-workspace/messages` create a query message, decide the target role names, expand each selected name to every matching concrete role id, and write one queued DB dispatch task per role id; the HTTP request does not start Codex/Hermes directly. A message can have both display `text` and an outbound `query`; a user query is represented as `role_id=user`, empty `text`, and non-empty `query`. Query text may start with one or more contiguous role-name mentions like `@Writer @Reviewer `; only this leading prefix is parsed for explicit dispatch targets, a mention selects all roles with that mention key/name, and any later `@` in the query body is ordinary text. On backend startup, the Super Workspace runtime ensures an independent `python -m backend.app.super_workspace_worker` process is running and registered under `WEAVER_RUN_DIR/worker.pid`. The worker process claims queued dispatch tasks only when the concrete target role id is idle, creates/resumes that role id's provider session, starts the detached provider driver, and updates task/query status from queued to running to completed/failed. Multiple roles with the same display name can run concurrently because serialization is per role id, not per name. Provider output rows are written only by the provider driver process/watcher with query/dispatch/parent lineage fields; the runtime and history read APIs do not perform fallback resyncs from Codex rollout JSONL or Hermes state, so missing DB messages remain visible as driver ingestion faults. `/api/super-workspace/events` streams lightweight Super Workspace SSE notifications for changed query messages; independent workers notify the backend through `/internal/super-workspace/notify`, and `SuperWorkspacePage.vue` calls `/api/super-workspace/runs?after=...` on events with a 30-second incremental fallback fetch. The Super Workspace page scrolls to the newest message on initial history load and preserves bottom stickiness for live updates. `/api/super-workspace/dispatch` remains as the raw role-router endpoint and sends the user message plus candidate role descriptions to an OpenAI-compatible chat-completions endpoint expecting JSON role ids; selected ids are expanded to all roles sharing those selected names before dispatch-task rows are written. Dispatch defaults to DeepSeek direct API model `deepseek-v4-flash`, reads `VIEWER_SUPER_DISPATCH_API_KEY` / `DEEPSEEK_API_KEY` from the process environment or the project-local ignored `.viewer.env`, and can be overridden with `VIEWER_SUPER_DISPATCH_MODEL` or `VIEWER_SUPER_DISPATCH_URL`.
...
- `SuperWorkspaceEventHub` streams lightweight run-created/run-updated notifications through `/api/super-workspace/events`; the history DB remains the source for actual messages, and the frontend uses the run `updated_at` cursor with `/api/super-workspace/runs?after=...` to fetch changed runs instead of reloading the newest page after every event.
...
- `list_super_runs()` / `get_super_run()` return DB-only lazy pages of non-empty-query messages with dispatch-task targets. New provider message rows are selected by `driver_run_id`; older rows without explicit linkage still fall back to the legacy prompt/time-window association. Reads do not reopen Codex rollout JSONL or Hermes state.
...
- Uses focus snapshots from the shared agent API for role status chips, while run/message history comes from the agent history DB through lazy `/api/super-workspace/runs` pages. SSE-triggered incremental fetches update the loaded history window when the backend reports changed runs; the frontend does not request provider resyncs. The page renders each persisted run as a chat-like block with the dispatch state shown as small muted metadata under the user query; role response labels sit above each message box, and only the final `message:assistant` text for each target is rendered through the shared Markdown renderer so reasoning/tool/thinking rows stay hidden.
...
- `~/.view/agent-history.sqlite3`: shared agent history index used first by Super Workspace and managed through SQLAlchemy ORM sessions with `NullPool` connection handling. It stores Super Workspace user messages/runs/targets plus provider driver-written intermediate message rows with full raw JSON, structured IR columns (`event_index`, `received_at`, `event_type`, `text`, `patch_text`, and `file_changes` child rows), provider/session/source path/line metadata, and source-derived timestamps. Runtime read/status paths do not resync or reparse provider source files as a fallback.
