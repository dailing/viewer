...
- Go 1.26 implementation line for the Viewer microkernel. The assembled `cmd/viewerd` now includes the original seven resident core plugins plus the M6a chat plugin and the embedded frontend; its CLI separates public-capable gateway `--host` / `--port` from loopback-default kernel `--kernel-host` / `--kernel-port`, provides `--data-dir`, and supports a development `--static` override.
...
- `internal/pluginapi/` owns the in-process plugin contract, compile-time eight-plugin registry, inspector-first ordered startup, reverse shutdown, loopback kernel-WS wiring, startup deadlines, and plugin-level panic isolation. `config.json` and `instance.json` live under the selected data directory; supervisor remains resident and manages only registry-listed external processes.
...
- `internal/plugins/terminal/` is the reusable Go terminal plugin: it owns PTY process sessions, incremental UTF-8 decoding, 30 ms/128 KiB output coalescing, the bounded in-memory snapshot ring, retained status replay after reconnect, 30-second post-exit history retention, resize/write/RPC handling, and whole-session process-group termination.
...
- `internal/acp/` implements the bounded NDJSON JSON-RPC ACP stdio client used by Go chat, including initialize/new/load/prompt/cancel, response-id correlation, raw session/update dispatch, stderr diagnostics, malformed-line tolerance, and process-exit failure propagation. `internal/codexserver/` implements the separate Codex App Server JSONL stdio subset: initialize/initialized, thread start/resume, turn start/interrupt, completion waiting, bounded 16 MiB frames, explicit rejection of server requests, and byte-exact notification delivery. `internal/plugins/chat/` implements the M7 Super Workspace plugin: workspace/Role/routing configuration is read and written through config-store under `plugins.viewer-chat`; Hermes ACP and Codex App Server are selectable Role drivers while OpenCode remains rejected; chats/messages/turns/turn summaries/chat×Role session triples live in GORM/modernc `chat.sqlite3`; every driver update first appends a complete raw row to `turn_events`, `events.go` synchronously derives a normalized `message_blocks` row, and only assistant text continues into the existing `messages` table and `chat:{id}:message` view. Tool, file-change, and command block summaries are interleaved into turn-summary transcripts while thinking blocks remain excluded. Explicit or one-shot OpenAI-compatible LLM routing feeds a strictly ordered multi-Role relay; completed turns are summarized asynchronously; and new/reused sessions receive budgeted summary, unsummarized-tail, role-switch, and optional Hindsight recall context. It keeps the M6a `send-message`/`stop` slot names as aliases while the primary submission contract is `chat:_:dispatch`.
...
- `scripts/mock_acp_agent.py` is a deterministic, no-LLM ACP stdio peer for initialize/new/load/prompt/update/cancel. `scripts/mock_codex_server.py` is the equivalent deterministic Codex App Server peer used by the Go protocol tests, so verification never launches real Codex. `scripts/migrate_agent_history.py` performs the idempotent one-shot read-only/online-backup migration from the production history schema into `chat.sqlite3` and includes a temporary SQLite fixture self-test. `scripts/smoke_chat.py` launches isolated config-store + chat plugins, creates two Roles and one chat, activates its retained mailbox, verifies the explicit dual-Role relay/event sender order and chat×Role DB rows, and checks the unconfigured-router error; `smoke_all.sh` includes it as the ninth suite.
- `scripts/smoke_single_binary.py` starts one assembled `viewerd` on isolated ports/data, checks embedded HTTP, all eight resident registrations through gateway `/ws`, both JSON stores, terminal IO, inspector capture, direct-kernel external-plugin compatibility, clean SIGTERM, and restart persistence. `web/build-release.sh` builds `next/frontend`, synchronizes its dist into the embed tree, and builds the release binary; `scripts/build-release.sh` remains the compatibility entry point.
...
- M6c chat frontend module registered through the same in-repo loader. `ChatPane.vue` renders a per-chat interleaved timeline without automatic scrolling, explicit/automatic dispatch controls, active-turn state, stop, and links to the management panes. `ChatsPanel.vue` owns chat CRUD/pinning/activation, `RolesPanel.vue` selects Hermes ACP or Codex App Server while leaving OpenCode unavailable, and `RoutesPanel.vue` provides validated JSON routing-policy editing. Shared bus payload mirrors live in `types.ts`.

`next/ts-sdk/tests/frontend-contract.test.ts`

- Go-backed Vitest contracts for every bus call used by the next/frontend terminal, Files, Bus Inspector, and chat panes, including chat Role/chat CRUD, active-mailbox replacement, explicit ordered dual-Role relay sender frames, busy rejection, and router missing-config behavior.
