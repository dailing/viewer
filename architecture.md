...
- `internal/acp/` implements the bounded NDJSON JSON-RPC ACP stdio client used by Go chat, including initialize/new/load/prompt/cancel, response-id correlation, session/update dispatch, stderr diagnostics, malformed-line tolerance, and process-exit failure propagation. `internal/plugins/chat/` is the M6a single-provider Hermes chat skeleton: one ACP child/session per chat, rejected concurrent turns, asynchronous message/completion events, and GORM/modernc SQLite persistence in `chat.sqlite3` with persisted provider-session load-or-recreate recovery.
...
- `scripts/mock_acp_agent.py` is a deterministic, no-LLM ACP stdio peer for initialize/new/load/prompt/update/cancel. `scripts/smoke_chat.py` launches an isolated chat-only `viewerd`, verifies the streamed bus turn contract, same-chat session reuse, GORM SQLite rows, and long-turn cancellation; `smoke_all.sh` includes it as the ninth suite.
