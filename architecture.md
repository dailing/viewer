...
- `SuperAgentDriver` is the provider-driver base. It checks the current chat+role backing session from `super_workspace_chat_role_sessions`, creates a clean provider session when missing/stale/cwd-or-model-mismatched/context usage is at least 70%, and starts the first role turn with common prompt plus fixed role rules plus optional recent visible chat history plus the routed query. Later tasks resume the existing chat+role provider session with only the routed query. Queueing is represented exclusively by DB task rows.
