...
- Go 1.26 implementation line for the Viewer microkernel. Milestone 4 assembles the kernel, all seven resident core plugins, and the embedded frontend into `cmd/viewerd`; its CLI separates public-capable gateway `--host` / `--port` from loopback-default kernel `--kernel-host` / `--kernel-port`, provides `--data-dir`, and supports a development `--static` override.
...
- `internal/pluginapi/` owns the in-process plugin contract, compile-time seven-plugin registry, inspector-first ordered startup, reverse shutdown, loopback kernel-WS wiring, startup deadlines, and plugin-level panic isolation. `config.json` and `instance.json` live under the selected data directory; supervisor remains resident and manages only registry-listed external processes.
...
- `scripts/smoke_terminal.py` is the Python-SDK black-box terminal suite used unchanged against Go and Python kernels with the Go terminal plugin.
