...
- `plugins/settings/` — unified full-page Settings pane (`settings:main`, opened from the Dock gear): layout open mode (`stores/layout.ts` `openMode`), chat reading virtual space (`stores/chatSettings.ts`), Dock hover-expand delay (`stores/dockSettings.ts`), app theme management (`stores/theme.ts`), markdown theme overrides (`stores/markdownStyle.ts`), and backend restart / build-restart via the gateway admin API. All toggles are browser-local (localStorage) and apply immediately. The theme system (`viewer.themes.v1`) ships built-in Light/Dark themes; each theme is a complete set of the shell's `--color-*` variables plus a light/dark base scheme, applied as inline custom properties on `.app-shell` with the scheme mirrored to `data-theme` (driving `color-scheme` and the styles.css Markdown color fallback). Built-in themes can be edited and reset but not deleted; custom themes duplicate the active theme and can be renamed, edited, and deleted. The Dock foot also has a pin toggle (`stores/dockSettings.ts` `pinned`): pinned keeps the Dock expanded and its width takes real layout space (compressing the panes) instead of hover-overlaying them. The expanded width (pinned and hover-overlay alike) is adjustable by dragging the Dock's right edge and persists via `stores/dockSettings.ts` `width` (default 220px, clamped to 140–520px).
