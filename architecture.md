...
- `/api/super-workspace` routes: per-user Super Workspace common prompt, role storage, persisted message/query history, and LLM dispatch. Role definitions live in `~/.view/users/{user}/super-workspace.json` and contain role name, fixed rules/description, provider, cwd, model, and the current backing agent session ref; the same file stores a `common_prompt` prepended to each new role session's initial prompt. `/api/super-workspace/runs` list-paginates Super Workspace messages whose `query` field is non-empty, backed by `~/.view/agent-history.sqlite3`; POST `/api/super-workspace/runs` and `/api/super-workspace/messages` create a query message and enqueue it into the backend Super Workspace runtime. A message can have both display `text` and an outbound `query`; a user query is represented as `role_id=user`, empty `text`, and non-empty `query`. Query text may start with one or more contiguous role mentions like `@Writer @Reviewer `; only this leading prefix is parsed for explicit dispatch targets, and any later `@` in the query body is ordinary text. Normal dispatch records backend-created driver runs after it binds selected roles to concrete agent sessions. Listing runs syncs targeted Codex/Hermes sessions into the Super Workspace message DB before returning the latest page. `/api/super-workspace/events` streams lightweight Super Workspace SSE notifications for changed query messages. `/api/super-workspace/dispatch` remains as the raw role-router endpoint and sends the user message plus candidate role descriptions to an OpenAI-compatible chat-completions endpoint expecting JSON role ids. Dispatch defaults to DeepSeek direct API model `deepseek-v4-flash`, reads `VIEWER_SUPER_DISPATCH_API_KEY` / `DEEPSEEK_API_KEY` from the process environment or the project-local ignored `.viewer.env`, and can be overridden with `VIEWER_SUPER_DISPATCH_MODEL` or `VIEWER_SUPER_DISPATCH_URL`.
...
- Backend Super Workspace dispatcher and first provider-driver layer.
...
- `SuperWorkspaceRuntime.submit()` parses leading `@Role ` query prefixes, persists the query message without the dispatch prefix, auto-dispatches through `SuperWorkspaceManager.dispatch()` when no explicit mentions are supplied, marks the run queued, and enqueues one per-role work item. Role mention keys are derived from role names using ASCII variable-name characters so frontend insertion and backend parsing agree.
- Maintains one in-process asyncio queue per role id so current Super Workspace dispatch is serialized per role. After sending or queueing the routed prompt, the role worker polls the provider session, syncs provider messages into the history DB, publishes lightweight SSE notifications, and only releases the next same-role query once the session is no longer running and its queue is empty. This is intentionally simple; the DB run state is the persistent source of truth and can later be claimed by a separate process/queue worker.
- `SuperAgentDriver` is the provider-driver base. It ensures a role has a valid backing `provider:session_id`, creates a fresh session with common prompt plus fixed role rules when missing/stale, sends the routed prompt when the session is idle, or queues it when the session is already running.
...
- `SuperWorkspaceEventHub` streams lightweight run-created/run-updated notifications through `/api/super-workspace/events`; the history DB remains the source for actual messages.
...
- Defines `super_workspace_messages`, `super_workspace_driver_runs`, `super_workspace_message_file_changes`, and `super_workspace_driver_checkpoints`. `super_workspace_messages` stores one intermediate-representation row per indexed message/event with scalar IR fields mapped directly to columns: `event_index`, `received_at`, `event_type`, `text`, `query`, and `patch_text`, plus provider/session/source path/line metadata, source-derived `occurred_at`, role, full provider `raw_json`, query/dispatch status, and driver-run association. `super_workspace_message_file_changes` stores the IR `file_changes[]` array as child rows with `path`, `change_type`, and `diff` columns instead of embedding it as JSON. `raw_preview` is not stored as an IR JSON blob because it is derivable from `raw_json`.
...
- `create_super_run()` records each Super Workspace user query as a `super_workspace_messages` row with provider `super_workspace`, empty display `text`, non-empty `query`, and selected role ids stored on that same row so direct and auto dispatch can return selected ids before driver runs exist.
- `record_super_target()` is the internal backend-driver hook for creating a `super_workspace_driver_runs` row. It binds one query message to a selected role's concrete `provider:session_id`, records the exact routed agent prompt, and stores a session time cursor so provider messages can be associated back to this driver run.
- `list_super_runs()` / `get_super_run()` return lazy pages of non-empty-query messages with driver-run messages. Driver message lookup syncs Codex/Hermes sources, associates non-user provider messages after the driver-run start cursor and before the next driver run in the same provider session, and caps returned messages per driver run.
- `sync_session()` delegates to provider drivers. Codex sync reads the viewer `CodexSession` rollout-backed raw events, reuses the existing Codex compact IR mapping, stores full raw JSON, and uses raw ids when present or `rollout_path:line` as a stable source id. Hermes sync reads canonical `~/.hermes/state.db` message rows, stores the row as raw JSON, and maps user/assistant/tool rows into the same IR shape.
...
- Loads per-user Super Workspace data through `/api/super-workspace`, edits common prompt plus role name/provider/cwd/model/fixed rules, and stores the current backing agent session ref on each role.
...
- Uses focus snapshots from the shared agent API for role status chips, while run/message history comes from the agent history DB through lazy `/api/super-workspace/runs` pages. Lightweight polling refreshes snapshots and reloads the currently loaded history window so long-running Codex/Hermes sessions show newly synced output after server restarts or frontend reloads. The page renders each persisted run as a chat-like block with selecting/dispatched role state, selected target chips, model rationale, role status, and DB-backed post-dispatch assistant messages rendered incrementally through the shared Markdown renderer.
...
- Super Workspace APIs: `getSuperWorkspace()`, `updateSuperWorkspace()`, role CRUD helpers, `listSuperWorkspaceRuns()`, `createSuperWorkspaceRun()`, and the raw `dispatchSuperWorkspace()` wrapper cover role storage, persisted query history, and LLM routing. Normal role-message delivery is owned by the backend Super Workspace runtime, not by frontend shared agent session helpers.
...
- TypeScript mirror of Super Workspace storage, persisted run history, and dispatch response shapes: common prompt, `SuperRole`, workspace/role create/patch payloads, `AgentHistoryMessage`, `SuperHistoryRun`, `SuperHistoryTarget`, paginated run responses, and selected route ids/rationale.
...
- `~/.view/users/{user}/super-workspace.json`: Super Workspace common prompt and role definitions, including fixed role rules, provider/cwd/model, and current backing agent session refs.
- `~/.view/agent-history.sqlite3`: shared agent history index used first by Super Workspace. It stores Super Workspace user messages/runs/targets plus synced Codex/Hermes intermediate message rows with full raw JSON, structured IR columns (`event_index`, `received_at`, `event_type`, `text`, `patch_text`, and `file_changes` child rows), provider/session/source path/line metadata, and source-derived timestamps. The DB uses a fast-moving schema version and may drop/rebuild these early history tables during local iteration; Codex rollout JSONL and Hermes `~/.hermes/state.db` remain source-of-truth inputs that can be resynced/reparsed.
