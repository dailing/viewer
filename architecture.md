...
- Go 1.26 implementation line for the Viewer microkernel. The assembled `cmd/viewerd` includes ten resident plugins, including chat and the headless `viewer.agent-hermes` / `viewer.agent-codex` services, plus the embedded frontend; its CLI separates public-capable gateway `--host` / `--port` from loopback-default kernel `--kernel-host` / `--kernel-port`, provides `--data-dir`, and supports a development `--static` override.
...
- `internal/pluginapi/` owns the in-process plugin contract, compile-time ten-plugin registry, inspector-first ordered startup, reverse shutdown, loopback kernel-WS wiring, startup deadlines, and plugin-level panic isolation. `viewer.agent-hermes` and `viewer.agent-codex` start after config-store and before chat so their catalog mailboxes are ready for routing. `config.json` and `instance.json` live under the selected data directory; supervisor remains resident and manages only registry-listed external processes.
...
- `internal/plugins/chat/` is the provider-agnostic Super Workspace orchestrator: workspace/Role/routing configuration is read and written through config-store under `plugins.viewer-chat`; its C1 `agents` mapping defaults `hermes` and `codex-app-server`, catalog mailboxes determine online policy candidates, and `chat:_:agent-catalog` aggregates online/offline entries. Every agent start/prompt/cancel and event/turn-ended exchange crosses the bus; chat has no ACP or Codex App Server import, parser, process, or in-process agent interface. Ordered routing-policy candidates honor `enabled`, `auto_failover`, and `max_attempts`; start/prompt RPC errors and `turn-ended:error` advance only when failover is enabled, and each `turn-completed` payload carries ordered attempt outcomes. Roles without a policy synthesize one runtime candidate from legacy provider/model fields without writing configuration. Chats/messages/turns/turn summaries/chat×Role session triples remain in GORM/modernc `chat.sqlite3`; bus event frames append raw `turn_events`, persist their supplied normalized `message_blocks`, and project only agent-text blocks into `messages`.
...
- `scripts/mock_acp_agent.py` is a deterministic, no-LLM ACP stdio peer for initialize/new/load/prompt/update/cancel. `scripts/mock_codex_server.py` is the equivalent deterministic Codex App Server peer, including catalog discovery and an injected start failure, so verification never launches real Codex. `scripts/migrate_agent_history.py` performs the idempotent one-shot read-only/online-backup migration from the production history schema into `chat.sqlite3` and includes a temporary SQLite fixture self-test. `scripts/smoke_chat.py` launches isolated config-store + both agent plugins + chat, asserts both catalog mailboxes and the aggregate catalog RPC, routes two Roles through an explicit Hermes policy, verifies Codex start/prompt/event persistence/turn-ended over the bus, checks ordered failover attempt details, verifies raw event/parsed block/DB behavior, and exercises asynchronous cancellation plus idempotent stop; `smoke_all.sh` includes it as the ninth suite.
...
- Chat frontend module registered through the same in-repo loader. `ChatPane.vue` renders a per-chat interleaved timeline without automatic scrolling, explicit/automatic dispatch controls, active-turn state, stop, and links to the management panes. `ChatsPanel.vue` owns chat CRUD/pinning/activation. `RolesPanel.vue` and `RoutesPanel.vue` fetch `chat:_:agent-catalog` and provide linked Agent/provider/model selectors with offline Agents disabled; route candidate parameters remain JSON text. Shared bus payload mirrors live in `types.ts`.

`next/ts-sdk/tests/frontend-contract.test.ts`

- Go-backed Vitest contracts for every bus call used by the next/frontend terminal, Files, Bus Inspector, and chat panes, including the real aggregated Agent catalog RPC, chat Role/chat CRUD, active-mailbox replacement, explicit ordered dual-Role relay sender frames, busy rejection, and router missing-config behavior.
