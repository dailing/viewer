...
- Defines `CONFIG_DIR`, `DATA_DIR`, `CONFIG_PATH`, `LOG_DIR`, `CODEX_LOG_DIR`, `HERMES_LOG_DIR`, `OPENCODE_LOG_DIR`, `TERMINAL_LOG_DIR`, `CODEX_RUN_DIR`, `HERMES_RUN_DIR`, and `WEAVER_RUN_DIR`.
...
- Defines local API models for `SuperWorkspaceData`, `SuperRole`, routing config, role update payloads, and dispatch responses. Routing entities live in config: provider accounts are non-secret credential references, ordered candidate targets bind runtime/account/model/parameters, and routing policies provide failover/max-attempt/cooldown behavior.
- `SuperWorkspaceManager.read()` / `update()` / create/update/delete role methods persist the common prompt, dispatcher-facing role descriptions, Agent-facing role prompts, route selection, and capability requirements. Effective route precedence is Chat Role override, then Role policy, then workspace default. Candidate eligibility applies enabled account state and declared tools/filesystem/context-window capabilities. Dispatcher role JSON/table rendering includes `description` but intentionally excludes `prompt`. Provider session refs are not role fields; current reusable sessions are owned by chat+role+runtime state rows in the history DB; changing a role prompt or route clears those mappings so the next dispatch creates a session with the new rules.
...
- The worker resolves the effective routing policy when it claims each task, walks eligible candidates in configured order, and preserves one worker-owned turn id across attempts. Credit/rate-limit/auth/model/runtime failures may advance to the next target when automatic failover is enabled; request and refusal failures stop instead of replaying the user action. Candidate cooldowns are worker-memory health hints, not billing state. The chosen immutable execution target and every attempt outcome are persisted on `super_workspace_driver_runs` for audit/display.
...
- `SuperAgentDriver` is the provider-driver base. It checks the current chat+role+runtime backing session from `super_workspace_chat_role_sessions`, creates a clean provider session when missing/stale/cwd-or-model-mismatched or when context usage reaches the Role override / configured provider percentage or token limit, and starts the first role turn with common prompt plus fixed role rules plus optional recent visible chat history plus the routed query. Later tasks resume the existing chat+role+runtime provider session with only the routed query. Queueing is represented exclusively by DB task rows.
...
- Defines `super_workspaces`, `super_workspace_user_state`, `super_workspace_roles`, `super_workspace_chats`, `super_workspace_chat_pins`, `super_workspace_chat_role_sessions`, `super_workspace_messages`, `super_workspace_driver_runs`, `super_workspace_message_file_changes`, `super_workspace_message_citations`, and `super_workspace_driver_checkpoints`. Roles store policy ids and capability requirements; chats store per-Role policy overrides. `super_workspace_driver_runs` stores the effective policy id, immutable execution-target JSON, and ordered routing-attempt JSON in addition to its role/session/task fields. The remaining table ownership and AgentEvent IR shape are unchanged.
...
- Tools: Chats, Roles, Routes, Files, Changes, and Terminals. `RoutesPanel.vue` edits the workspace default, ordered routing policies, candidate runtime/account/model/capability declarations, and non-secret provider account references. Future side tools should be added to this shell instead of mixing unrelated lists into one panel.
...
- `VIEWER_CONFIG_DIR/config.json` (legacy default `~/.view/config.json`): appearance, Codex, voice, Markdown, routing policies/accounts, and Super Workspace configuration, managed by `/api/config` and `/api/super-workspace/routing`.
