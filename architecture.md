...
- Go 1.26 implementation line for the Viewer microkernel. The assembled `cmd/viewerd` includes nine resident plugins, including chat and the headless `viewer.agent-hermes`, plus the embedded frontend; its CLI separates public-capable gateway `--host` / `--port` from loopback-default kernel `--kernel-host` / `--kernel-port`, provides `--data-dir`, and supports a development `--static` override.
...
- `internal/pluginapi/` owns the in-process plugin contract, compile-time nine-plugin registry, inspector-first ordered startup, reverse shutdown, loopback kernel-WS wiring, startup deadlines, and plugin-level panic isolation. `viewer.agent-hermes` starts after config-store and before chat so its catalog mailbox is ready for routing. `config.json` and `instance.json` live under the selected data directory; supervisor remains resident and manages only registry-listed external processes.
...
- `internal/agentdriver/` defines the provider-neutral headless-agent bus types and normalized block-kind constants. `internal/acp/` implements the bounded NDJSON JSON-RPC ACP stdio client and owns ACP update-to-block parsing through `ParseBlock`; it includes initialize/new/load/prompt/cancel, response-id correlation, raw session/update dispatch, stderr diagnostics, malformed-line tolerance, and process-exit failure propagation. `internal/plugins/agenthermes/` is the single-instance `viewer.agent-hermes` service plugin: it reads the existing Hermes command/profile/YOLO environment controls, owns the session-to-ACP-subprocess pool and load fallback, immediately acknowledges prompts, publishes ordered raw-plus-parsed event frames and turn-ended events, and sets a best-effort/overridable retained catalog under `viewer.agent-hermes:_:catalog`.
- `internal/codexserver/` implements the separate Codex App Server JSONL stdio subset and remains an in-process chat driver until M8b. `internal/plugins/chat/` is the Super Workspace orchestrator: workspace/Role/routing configuration is read and written through config-store under `plugins.viewer-chat`; its C1 `agents` mapping defaults Hermes and future Codex plugin ids, catalog mailboxes determine online policy candidates, `chat:_:agent-catalog` aggregates online/offline entries, and start/prompt/cancel plus event/turn-ended traffic crosses the bus for Hermes. Ordered routing-policy candidates honor `enabled`, `auto_failover`, and `max_attempts`; Roles without a policy synthesize one runtime candidate from legacy provider/model fields without writing configuration. Chats/messages/turns/turn summaries/chat×Role session triples remain in GORM/modernc `chat.sqlite3`; bus event frames append raw `turn_events`, persist their supplied normalized `message_blocks`, and project only agent-text blocks into `messages`. Relay, summaries, context bridges, DB schemas, and the temporary Codex path otherwise retain their behavior.
