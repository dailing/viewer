...
- `internal/acp/` implements the bounded NDJSON JSON-RPC ACP stdio client used by Go chat, including initialize/new/load/prompt/cancel, response-id correlation, session/update dispatch, stderr diagnostics, malformed-line tolerance, and process-exit failure propagation. `internal/codexserver/` implements the separate Codex App Server JSONL stdio subset: initialize/initialized, thread start/resume, turn start/interrupt, completion waiting, bounded 16 MiB frames, explicit rejection of server requests, and notification delivery. `internal/plugins/chat/` implements the M6c Super Workspace plugin: workspace/Role/routing configuration is read and written through config-store under `plugins.viewer-chat`; Hermes ACP and Codex App Server are selectable Role drivers while OpenCode remains rejected; chats/messages/turns/turn summaries/chat×Role session triples live in GORM/modernc `chat.sqlite3`; explicit or one-shot OpenAI-compatible LLM routing feeds a strictly ordered multi-Role relay; completed turns are summarized asynchronously; and new/reused sessions receive budgeted summary, unsummarized-tail, role-switch, and optional Hindsight recall context. It keeps the M6a `send-message`/`stop` slot names as aliases while the primary submission contract is `chat:_:dispatch`.
