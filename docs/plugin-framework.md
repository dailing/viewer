...
> 状态：**草案 v0.51**（2026-08-22）。本文档是架构决策的唯一权威来源，逐节评审、迭代定稿。只记录已决定的内容，不记录决策过程。**线路级协议规范见 `docs/plugin-protocol.md`（Phase 0，冻结后写码）。**
...
- **LLM 转发层**：core plugin `llm`（§10.2 C6）——唯一 RPC `llm:_:complete {messages, json_mode?, timeout_seconds?, extra_body?} → {content, model}`，纯转发到 OpenAI 兼容端点；调用方插件不感知模型/端点/密钥。`extra_body`（v0.49）原样并入出站请求体，用于端点特定开关（如 `chat_template_kwargs.enable_thinking=false`）；严格端点可能拒为 400，由调用方按 `llm_error` 呈现。配置 = config-store `plugins.llm`（`active` 当前配置 + `profiles` 前端管理配置库，逐调用重读、改即生效；超时优先级：请求参数 > 配置 > 默认 60s）；设置 UI = llm 插件自己的 singleton Dock pane（自 chat-manager 模型 tab 迁出）。启动时**一次性迁移** `plugins.viewer-chat.llm`→`plugins.llm.active`、`llm_profiles`→`profiles`（仅当目标为空）；chat 的 dispatch router / turn summary / voice speakable 全部改走此 RPC，`plugins.viewer-chat.llm*` 废弃。未配置时回 RPC error code `not_configured`，调用方据此给出口语化提示。
