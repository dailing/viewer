...
> v0.66 变更：**分支重定位为纯 context 分区 + 手动归档（by the way 模式）**——①分支不再绑定属主角色：chat 视为 working group，分支只是其中一个"私聊分隔间"；`branch_id` 派发的角色选择与主线**完全一致**（显式 role_ids 或 LLM 路由、可多 role；`force_new_session`/`parallel_dispatch` 均生效，v0.63 起的互斥置空取消），唯一区别是 context 与 session 按分支隔离：每个 role 续接自己在该分支上的 session lane（role × branch 键 `chat\x00role\x00branch\x00<branch>`，resume 源 = 该 role 在分支上最新 turn 的 session），无 lane 或 force_new 时开 fresh session 并以分支 lineage 构建 context——**开分支 = 带 lineage 上下文的新 chat**，write/review 双分支这类模式由此直接成立。②新增 `chat:_:branches:archive` `{chat_id, branch_ids}`：**不合并直接归档**（在途 turn 拒绝、已归档报错、空分支可归档）；archive-only 行无 `merge_message_id`，`branchesMergedInto` 谓词加 `merge_message_id != ''`，其 turns 不进入任何线的 lineage——by-the-way 旁支问答与主线 context 完全隔离；**恢复（unarchive）为保留功能，暂不实现**（归档行仍在 `已归档` 里只读可查）。③`merge-confirm` 目标为分支时，摘要钉给该分支最新 turn 的 role（仍为 chat 成员时，显式 role_ids），不再落入 LLM 随机路由。④前端：分支上 composer 行为与主线完全一致（角色选择器不再置灰、rolePickHint 机制移除、new session/send-now 开关生效）；「发送到」标注统一为「分支名 → 所选角色或自动路由」；分支条在选中集合含活跃分支时出现「归档」按钮（confirm 后归档并回主线）；「已归档」同时收纳合并与手动归档分支。
> v0.65 变更：**分支条交互修订 + 空分支首轮角色选择生效**——①分支条 tab 改**单击单选**（查看/锁定发送目标），Ctrl/⌘+点击才进入多选（多选集合即合并选择，规则不变：含主线合主线、否则合进最先点选者）；「全部」仅为查看全部线，不再构成合并选择（v0.64 的常开多选取消）。②分支条排序定稿：全部 → 主线 → 各活跃分支 → 已归档。③**空分支首轮尊重 composer 角色选择**：branch_id 派发在前端补上 role_ids（分支尚无 turn 时），显式 role_ids 跳过 LLM 路由、首轮即定分支属主；分支已有 turn 后 role pick 不再发送（后端续接属主、本就会忽略）。④非空分支续接时 composer 角色选择器置灰并提示「角色固定为属主」（防误导）；「发送到」标注细分：分支续接显示属主角色、空分支首轮显示所选角色或自动路由。
> v0.64 变更：**分支 fork 点 + turn 父链 + 多选合并（分支 DAG 化）**——①`turns` 表新增 `prev_turn_id`（beginTurn 盖章：分支线取分支最新 turn、首轮取 fork turn；lane 续接取该 session 最新 turn；其余取主线最新 turn），turn 由此形成可回溯的 commit 链。②`branches` 表新增 `fork_turn_id`/`parent_branch_id`（分支点：允许从主线或任何活跃分支的任意 turn 分叉，多级成 DAG；归档分支的 turn 不可再分叉）与 `merged_into_branch_id`（合并目标，空 = 主线；旧归档行零值天然归属主线）。③`branches:create` 新增可选 `from_turn_id`（校验同 chat、所在线未归档），空名称默认 `分支NN` 自动编号。④合并目标泛化：`branches:merge`/`:merge-confirm` 新增 `target_branch_id`（空 = 主线），目标须活跃且不在被合列表；摘要 dispatch 进目标线（目标为分支时即该分支的续接 turn），源分支归档并记录 `merged_into_branch_id`。⑤context 构建改 lineage 口径：fresh 首轮与 role-switch bridge 的"上一条消息"沿本线 turn 集合回溯（本线 turns + fork 链上各父线截止 fork turn 的 turns + 递归并入 merged_into 本线的分支 turns，统一按时间排序），摘要+原始尾巴预算规则不变；合并后目标线的 context 把被合分支 turns 按时间交错并入，视同从未分叉。⑥前端：分叉入口从分支条 ＋ 移到每个 role turn 盒顶条的分叉按钮（归档线的盒不显示）；分支条 tab 改多选（点选切换、全部 = 全选、已归档只读可加入查看），选中集合即合并选择——含主线则合进主线，否则合进最先点选的分支；发送目标 = 恰有一个活跃分支被单独激活时续接它，否则主线。
> v0.63 变更：**命名分支 + 合并回主线（chat 并行工作线管理）**——①分支为独立持久化记录（`branches` 表：name/role/latest session/archived_at/merged_through_turn_id/merge_message_id），`turns` 表新增 `branch_id` 列（beginTurn 时盖章）——标签身份不再派生自 provider session，换模型/session 重建不会凭空多出标签。②`chat:_:dispatch` 新增 `branch_id`：有 turn 的分支续接其最新 session（branch 级 runtime/busy key `chat\x00role\x00branch\x00<branch>`），空分支首轮由选中 role 的第一位开 fresh session（分支单 role；与 force_new_session/parallel_dispatch 互斥置空）；匿名 `parallel_dispatch` 自动为每个启动 role 建分支（名取消息前 12 字符，多 role 后缀 role 名），reply 带 `branches`。③分支管理 RPC：`chat:_:branches:create`（即建即持久化）/`:patch`（重命名）/`:delete`（仅零 turn 未归档）；`chats:list`（带 chat_id）新增 `branches`（含已归档），新增 `chat:_:branch` feed（created/updated/archived/deleted），turn feed 各 phase 帧带 `branch_id`。④合并：`branches:merge` `{chat_id, branch_ids}` 校验（同 chat、未归档、非空、无在途 turn——运行中的分支等结束后再合并）后由 LLM 基于各 turn 摘要（缺摘要回退截断转录）生成**可编辑**合并摘要（各分支成果/代码改动与验证/分支间的冲突/待办）+ 每分支 cutoff turn；`branches:merge-confirm` `{chat_id, summary, branches:[{id, cutoff_turn_id}]}` 复核后把摘要作为普通 dispatch 送入主线，随后归档分支并记录 cutoff（避免重复导入）与 merge_message_id（关联主线摘要消息）。⑤前端分支条常驻 composer 上方：主线（默认）/分支 tab/全部/已归档 N/＋；激活分支 tab = 过滤 + 锁定续接，双击重命名；composer 上方常驻「发送到：主线/分支名」标注；合并模式多选 → 摘要编辑卡 → 确认归档；主线摘要消息带「已合并分支」卡片可展开查看原始对话（只读）。合并是对话成果合并——共享工作目录的代码冲突由摘要在「分支间的冲突」节列明。v0.60 的 lane 条被分支条取代（lane 续接 API `continue_turn_id` 保留）。
> v0.62 变更：**chat 记忆写入（retain）**——chat 插件把每条可见消息（dispatch 时的用户输入 + turn 完成时的各 role 最终回复）异步 retain 到 Chat 级 Hindsight bank，带真实时间戳与 provider/role/message id 元数据；per-chat bank 覆盖所有 role/agent/provider（agent 自有 memory 只见单个 agent 的 session）。首次 retain 前自动 PATCH bank 关 auto-consolidation；SQLite 仍是 source of truth，bank 是派生索引，retain 失败仅记日志。新增配置 `hindsight.retain_enabled`（默认 true）/`retain_timeout_seconds`（默认 30，服务端同步抽取）。
> v0.61 变更：**排队消息可取消/可原位编辑**——①chat 新增 `chat:_:queued-cancel`（`{chat_id, dispatch_id}`：摘除该 dispatch 的全部排队条目（多 role dispatch 每 busy role 一条）、删除其 user 消息行并在 `chat:{id}:message` 推 `{id, deleted:true}` 墓碑帧）与 `chat:_:queued-update`（`{chat_id, dispatch_id, message}`：原位改写排队条目文本 + user 消息行，**排队位置不变**）；条目已开始执行时两者均回 `not_queued`。②队列可观察：`chats:list`（带 chat_id）新增 `queued_messages` 快照（message_id/dispatch_id/role/position/enqueued_at），新增 `chat:_:queue` feed——每次队列变动（入队/出队/取消/编辑）推全量快照，pane 整量重seed。③前端 query box 顶条新增 queued chip（沙漏图标，与 running chip 同款两次点击确认取消）+ 铅笔按钮（文本载入 composer 进入编辑模式，下一次发送走 queued-update 原位更新；编辑中条目开始执行则 queue feed 自动退出编辑模式）。
> v0.60 变更：**session lane + lane 续接（chat 并行任务的可视化与续接）**——①`turns` 表新增 `session_id` 列（turn 拿到 runtime 后盖章），同一 provider session 的 turn 链 = 一条 lane；`chats:list`/`blocks:list` 新增 `turn_sessions`（turn_id → session/dispatch/role/started_at 映射），turn feed 新增 `phase:"session"` 实时帧。②`chat:_:dispatch` 新增 `continue_turn_id`：跳过 LLM 路由、派给该 turn 的 role、显式 resume 该 turn 的 session（lane 级 runtime/busy key `chat\x00role\x00lane\x00<session>`，与 canonical/throwaway key 互不抢锁；lane 恰为 role 的 canonical session 时回落 canonical 路径、stored 指针保持权威）；session 在飞时续接进 lane 级队列，在途 turn 结束由 relay 直接 kick 该 lane 队列（throwaway key 无 busy 锁可释）；续接显式覆盖 `new_each_run` 策略、永不改写 `role_sessions` 的 canonical 指针，与 `force_new_session`/`parallel_dispatch` 互斥（置空）。③前端 lane 条（≥2 条 lane 时出现在 composer 上方）：「全部」+ 每 lane 一个 tab（role 名+序号、running 带 spinner）；**激活 tab = 时间线过滤到该 lane 且下一条消息默认以 `continue_turn_id` 续接它**（看哪条接哪条）；并行 send-now 的 throwaway session 由此从一次性变为可寻址、可续接的 lane。
> v0.59 变更：**活动折叠口径放宽 + 浮动还原不再顶替**——①chat 第一级折叠计入全部小灰行活动（tool_call/tool_result/command/file_change/thinking/error），仅正文文本打断分组，连续 ≥3 条收成 `Tools ×N`；②浮动 pane 放回（`dockFloating`）不再经 `openInstance` 顶替现有内容：平铺三模式在叶端追加新 tile（末位 tile 为空则直接填入），自由模式替换 active pane。
> v0.58 变更：**持久化全局 input session + composer pin/overlay**——输入草稿、目标 plugin/pane/instance、角色选择、一次性发送选项与 pin 偏好从 Composer 组件提升到 `inputSessions` 全局 store，并写 `viewer.inputSessions.v1` localStorage；插件在激活期注册 send adapter，pane 存在时可注册 runtime sender 保留 optimistic UI，pane 关闭后仍由插件 adapter 投递原目标。voice context 同步持久化到 `viewer.voiceContexts.v1`，MediaRecorder 不再随 VoiceInputButton 卸载取消；workspace bar 显示 active input 的 draft/recording/processing/sending 状态及停止/发送按钮。发送录音中的 session 必须先 graceful stop、等待 voice final/LLM refine 后再 dispatch，成功才清稿并清 voice session，失败保留全部内容。Chat Composer 默认 pin 在底部；取消 pin 后失焦收成 pane 右下角半透明按钮。浏览器刷新无法延续 MediaRecorder，但刷新前草稿/partial transcript 恢复为可编辑 ready/idle 内容。
> v0.57 变更：**工具活动两层折叠 + 语义化详情 + Codex command 聚合**——同一 turn 内连续 3 条及以上 tool_call/tool_result/command/file_change 在前端收为一条稳定的 `Tools ×N` 行（文本、thinking、error 均打断分组）；展开第一层看逐项，逐项仍可展开第二层。单项外层按 execute/read/edit/search 等显示图标、类型与命令/路径摘要（220 字符封顶），内层按 Command/Path/Input/Arguments/Output/Changes 等字段格式化（总预算 9000 字符，超限显式省略），不再直出裸 JSON。ACP adapter 读取 Hermes 嵌套 `content[]` 和 locations；Codex adapter 把 commandExecution 的 started/outputDelta/completed 用 item id 归一为同一 activity，chat 侧聚合 stdout delta，完成态 aggregatedOutput 覆盖增量缓存，因此一次命令只占一条工具项。
...
> v0.51 变更：**语音对话历史按协议 JSON 存储**——voice-control 的 assistant 历史轮次一律存规范化 `{"say","entry_id"}` JSON（直答/失败记 `entry_id:"none"`，成功命令记实际条目 id），与系统模板要求的输出格式同构：v0.50 前历史里存的是纯朗读文本，模型从第二轮起模仿历史、丢掉 JSON 外壳，表现为「念了已打开聊天但没派发任何条目」（open_instance effect 未追加、pane 不开）；折叠压缩时摘要器拿到的仍是解包后的朗读文本。
> v0.50 变更：**语音控制可观测性 + prompt 模板可配置**——①**交互日志**：voice-control 内存 bounded ring（100 条）记录每轮交换（转写、LLM 原始输出、命中条目/invoke channel、effects、LLM/invoke 耗时、前端状态机相位——command RPC 新增 `phase` 字段上报）与状态事件（开关会话、上下文折叠、主动播报），经 retained mailbox `voice-control:_:log` 全量镜像（新订阅者立即拿到全量历史），另有 RPC `voice-control:_:log` 快照与 `voice-control:_:log:clear` 清空。②**prompt 模板可配置**：系统模板（占位符 `{{catalog_section}}`）与摘要模板（`{{summary}}`/`{{turns}}`）存 config-store `plugins.voice-control`（`system_template`/`summary_template`），逐调用重读、缺省/空值回退内置默认——改即生效，无需重启；RPC `voice-control:_:prompt {text?}` 返回生效模板 + 内置默认 + 当前会话摘要/历史 + 实时渲染的完整 messages 预览。③**debug pane**：voice-control 前端不再是纯 headless——新增 singleton Dock pane（bi-mic「语音控制」）三 tab：日志（live）/ Prompt（模板编辑 + 预览）/ 功能目录（merged catalog 按插件分组），头部常显状态机相位。
> v0.49 变更：**语音对话化 + catalog 频道名修复**——①**catalog mailbox 改三段频道名** `voice-catalog:_:<plugin>`（订阅 `voice-catalog:_:*`）：v0.48 的两段名 `voice-catalog:<plugin>` 触发 kernel `invalid_channel`（频道至少三段，§5.2），发布被异步拒绝、合并目录恒空——症状为语音只会回「还没有可语音操作的功能」。②**命令解析升级为连续语音对话**（§8.11）：`voice-control:_:command` 不再是一次性意图匹配，每句转写携带 系统规则 + 功能目录 + 对话摘要 + 最近轮次 调 `llm:_:complete`，模型输出 `{"say","entry_id"}`——`entry_id=none` 时直接朗读 `say`（可回答「有什么功能」等自由提问/闲聊），命中条目才 invoke（朗读插件返回的执行结果 say）。③**对话上下文与压缩**：voice-control 内存保存 per-session 历史（user/assistant 轮次）+ running summary；摘要+历史超过字符预算（3000）时把最旧轮次折叠进摘要（LLM 压缩，至少折一半、不够续折、恒保留最新 4 条；压缩失败直接丢弃最旧部分不阻塞对话；硬顶 40 条）；`voice-control:_:enable` 切换即重置会话。④**thinking 关闭**：`llm:_:complete` 新增 `extra_body`（原样并入出站请求体，端点特定，严格端点可能拒为 400）；voice-control 全部调用带 `chat_template_kwargs.enable_thinking=false`。
> v0.48 变更：**语音控制全局化 + LLM 转发层**——①新 core plugin `llm`（§10.2 C6）：唯一 RPC `llm:_:complete` 纯转发 OpenAI 兼容端点，配置 `plugins.llm`（`active` + 前端管理的 `profiles` 库，逐调用重读），设置 UI 迁为 llm 插件自己的 singleton pane，启动一次性迁移 `plugins.viewer-chat.llm*`（仅当目标为空）；chat 的 dispatch router / turn summary / voice 全部改走此 RPC。②新 core plugin `voice-control`（§10.2 C7，§8.11 重写）：语音控制从 chat pane 级提升为 **shell 全局连续 loop**——Dock-foot 新通用扩展点 `createDockActions`（图标/title/active 响应式 getter，任意插件可注册全局动作按钮）的耳机 toggle + 全局 Ctrl+Shift+M；条目发现 = `voice-catalog:*` retained mailbox（取代广播-应答，无启动时序窗口）；执行 = `entry.channel` RPC 返回 `{ok, say, effects[]}`，`open_instance` effect 由 voice-control 自动追加；插件私有 effect 经 retained mailbox `voice-fx:<plugin>:<instance>`（nonce 去重 + 30s 过期）交接目标 pane，口述/确认回路留在 pane 内、完成后 publish `voice-control:_:resume` 恢复 loop。③主动播报：订阅 `chat:_:turn`，loop 开启时 turn 完成自动念摘要（error 直接念失败，cancelled 静默），前端 toggle 同步 `voice-control:_:enable`。composer 耳机按钮删除；v0.47 的 pane 级命令控制器废弃。
> v0.47 变更：**语音命令 slice 1（§8.11 Voice Actions）**——chat 插件新增 RPC `chat:_:voice:command`：语音转写 → LLM 意图解析（catalog = 每聊天一条 open_instance 条目 + read-latest/dictate/stop/status 四个动作条目）→ 执行并返回 `say` 口语行 + effects；read-latest 经 LLM 改写为可朗读摘要（失败回退去代码围栏+截断）。前端 composer 新增语音命令按钮（快捷键 Ctrl+Shift+M）：录音复用既有 voice 链路（命令跳过 refine），端点检测 = 客户端静音检测（命令/确认 2s、口述 4s 尾静音自动结束），确认回路关键词（发送/取消）只在短确认语句上本地匹配，TTS = 浏览器 SpeechSynthesis。条目/效果/结果形状按跨插件通用设计，后续可提升为全局注册表而不改 chat 实现。
> v0.46 变更：**dispatch router 走共享 LLM helper + 失败落日志**——`routeWithLLM` 不再手写 HTTP，改调 `llm.go` 的 `chatCompletion`（endpoint 统一归一化：缺 `/chat/completions` 后缀自动补）。此前 router 裸 POST 到 `config.Endpoint`，配置为 `http://host/v1` 形式时打到 `/v1` 根路径必 404，症状为「不指定角色/模型必失败、指定即跳过路由成功」。router 失败现在写 `slog.Warn("chat dispatch routing failed")` 到 viewerd.log（此前只回 RPC 错误、服务端无痕）。
> v0.45 变更：**chat LLM 配置库 + dispatch 超时修复**——①模型 tab 改 §8.9 master-detail：config-store 新增 `plugins.viewer-chat.llm_profiles`（前端管理的配置库，首次打开播种 llama-server/Ollama 两条，删空不回填），active 仍是 `plugins.viewer-chat.llm`（逐调用重读、即时生效不变），新增 `timeout_seconds` 字段。②超时修复：共享 `http.Client` 去掉 30s 硬顶（此前把 summary 配置的 60s 静默截断为 30s），各调用方自管 ctx——router 用 `llm.timeout_seconds`（默认 60）、summary 用 `turn_summary.timeout_seconds`、hindsight 用 `hindsight.timeout_seconds`。
> v0.44 变更：**计划重启可取消 + 前端状态可见**——①新增 `DELETE /api/admin/schedule-restart`（v0.42「不设取消」语义废除）：armed/building/failed 一律归位 `none`，幂等；进行中的构建不杀进程、跑完但不再武装（building→armed/failed 转换改 CAS，取消后 CAS 失败即弃）。agent 经 HTTP 的完整闭环：POST 计划、GET 查询、DELETE 取消。②Settings 后端区「空闲时重启」改 toggle：按钮实时反映 GET 状态（mount + 5s 轮询，覆盖 agent 经 API 武装的计划），building/armed 态点击即取消，下方状态行常显四态文本；等待循环新增「状态归 none = 已被取消」退出路径。③Dock 底部新增计划重启指示器（15s 轮询 GET）：building/armed 警告色时钟图标、failed 危险色告警图标，点击打开 Settings。④flat 风格：Dock 侧边栏展开/固定态阴影移除（overlay 展开不再投影）。
> v0.43 变更：**计划重启带构建（取代 v0.42 的纯武装语义）**——`POST /api/admin/schedule-restart` 改为先在后台跑 `web/build-release.sh`（与 build-restart 共用同一构建守卫 `buildMu`，冲突返回 409），构建成功才武装等空闲看门狗：不构建直接重启只会加载同一旧二进制，重启无意义。构建失败不武装、状态落 `failed`；新增 `GET /api/admin/schedule-restart` 状态端点（`none|building|armed|failed`），前端据此区分「构建失败」与「等空闲」，失败即停止轮询并提示构建日志位置；`failed` 态可再次 POST 重新武装。状态机纯内存，重启即丢。同时落地 shell 浮动层：pane 可经抬头按钮提出 split 树成为浮动面板（拖动标题栏移动、右下角拉伸、点击置顶、可放回布局或关闭），Dock 点击已浮动实例改为置顶而非重复打开。
...
- **浮动放回语义（v0.59）**：`dockFloating` 不顶替任何已占用 pane——平铺三模式在叶端追加一个新 tile 放入（末位 tile 为空则直接用之）；自由模式替换 active pane 的内容（geometry 由用户手工管理）。
...
### 8.10 插件主题集成（v0.80 基色派生）

- **持久化（v0.81）**：主题**定义**存服务端 instance-store（plugin `shell` / instance `themes`，整值 `{themes:[...]}`），跨浏览器/机器共享，`instance-store:shell:themes` mailbox 实时同步 + 重连全量对账（server wins）；**活跃主题 id 浏览器本地**（`viewer.theme.active.v1`），每设备可不同；`viewer.themes.v2` 仅作启动缓存与一次性种子源。
- **主题变量单源**：主题 = 4 个基色（`--color-canvas` / `--color-surface` / `--color-text` / `--color-accent`）+ 高级覆盖，活动主题以 inline custom property 设置在 `.app-shell` 上；其余 `--color-*` / `--markdown-*` / `--syntax-*` 默认值由 `styles.css` 的 `.app-shell` 块用 `color-mix()` 从基色派生，`[data-theme="dark"]`（由 canvas 亮度自动判定）仅覆盖语义色、语法色板等 scheme 常量。插件渲染在 `.app-shell` 子树内，直接 `var(--color-*)` 即跟随活动主题与亮暗切换。
...
### 8.11 语音控制：Voice Actions（v0.49 对话化）

- **目标形态**：免手免屏盲操——Dock 底部耳机按钮 / Ctrl+Shift+M 开启**全局连续 loop**：听一句 → LLM 回复或执行并语音回读 → 自动回到听，直到说「退出语音」/「关闭语音」或再点按钮。控制权在 shell 全局层（voice-control 前端插件，随 shell activate 存活），pane 切换不丢控制权——v0.47 的 pane 级命令控制器废弃。
- **LLM 转发层**：core plugin `llm`（§10.2 C6）——RPC `llm:_:complete {messages, json_mode?, timeout_seconds?, extra_body?} → {content, model}` 纯转发到 OpenAI 兼容端点；调用方插件不感知模型/端点/密钥。`extra_body`（v0.49）原样并入出站请求体，用于端点特定开关（如 `chat_template_kwargs.enable_thinking=false`）；严格端点可能拒为 400，由调用方按 `llm_error` 呈现。配置 = config-store `plugins.llm`（`active` 当前配置 `{endpoint, key, model, timeout_seconds, extra_body?}` + `profiles` 前端管理配置库，逐调用重读、改即生效；超时优先级：请求参数 > 配置 > 默认 60s；`active.extra_body`（v0.54）为该模型的默认请求字段，并入每次出站调用、调用方逐 key 覆盖）；设置 UI = llm 插件自己的 singleton Dock pane（自 chat-manager 模型 tab 迁出）。启动时**一次性迁移** `plugins.viewer-chat.llm`→`plugins.llm.active`、`llm_profiles`→`profiles`（仅当目标为空）；chat 的 dispatch router / turn summary / voice speakable 全部改走此 RPC，`plugins.viewer-chat.llm*` 废弃。未配置时回 RPC error code `not_configured`，调用方据此给出口语化提示。HTTP facade 配置为 `plugins.llm.http {enabled,port,expose}`，由设置 pane 经 `llm:_:http:configure` 热更新，`llm:_:http:status` 返回 enabled/running/base_url/expose/last_error；`expose=false` 监听 `127.0.0.1`，true 监听 `0.0.0.0`，开放 `POST /v1/chat/completions`（透明转发并强制覆盖为 active model，支持 streaming）、`GET /v1/models` 与 `GET /health`。每个 completion 带 request ID 并结构化记录原始请求、实际转发、响应与耗时（返回记录限 4 MiB、Key 排除），供外部应用共享 active 切换与适配调试。
- **条目发现 = retained mailbox**：支持语音的插件 `Set("voice-catalog:_:<plugin>", {entries})`——mailbox 常驻最后值、新订阅者立即拿到全量，catalog 变化（聊天增删改）重发即推送；voice-control 订阅 `voice-catalog:_:*` 合并全局目录。相比「广播-应答」省掉 announce 协议与启动时序窗口，单一机制无特判。（v0.49 修复：两段名 `voice-catalog:<plugin>` 被 kernel `invalid_channel` 异步拒绝。）
- **条目形状**：`{id, plugin, kind: open_instance|action, title, keywords[], description, pane_type?, instance_id?, channel}`。`open_instance` 每实例一条（如每聊天 `open-chat:<id>`，keywords 含聊天名与 root basename），`action` 是实例内操作；`channel` 是该插件的执行入口 RPC。
- **对话链路（v0.49 重写）**：转写文本 → `voice-control:_:command {text}` → prompt = 系统规则 + 合并 catalog + 对话摘要 + 最近轮次 + 当前转写，调 `llm:_:complete`（json_mode + `extra_body` 关 thinking）→ 模型输出 `{"say","entry_id"}`（按语义容错转写噪声/中英混说，entry_id 校验回 catalog）：`entry_id=none` = 直接回答/闲聊，朗读 `say`；命中条目 → RPC 调 `entry.channel {entry_id, instance_id, text}` → 插件返回 `{ok, say, effects[]}`，朗读执行结果的 `say`；`kind=open_instance` 时 voice-control 自动追加 `open_instance` effect（插件不管前端）。**所有结果——含失败——都带 `say` 口语行由客户端朗读**，仅 payload 非法走 RPC 错误。
- **对话上下文与压缩（v0.49）**：voice-control 内存保存 per-session 历史（user/assistant 轮次；assistant 轮次按协议 JSON 存储，v0.51）+ running summary；摘要+历史字符数超预算（3000）时在下一次调用前把最旧轮次折叠进摘要（LLM 压缩 ≤200 字：至少折一半、仍超预算续折、恒保留最新 4 条；压缩失败直接丢弃最旧部分——降级上下文不得阻塞实时对话）；历史硬顶 40 条。`voice-control:_:enable` 切换（开/关）即重置会话，新会话从干净上下文开始；状态纯内存，重启即丢。
- **交互日志（v0.50）**：每轮交换与状态事件落内存 bounded ring（100 条）：交换记录转写、最终 say、LLM 原始输出、命中条目（id/title/channel）、effects、ok/fail、LLM/invoke 耗时、前端相位（command RPC 的 `phase` 字段）；事件记录开关会话（enable 切换重置上下文）、上下文折叠（成功/失败）、主动播报内容。retained mailbox `voice-control:_:log` 全量镜像（debug pane 订阅即 live，重开即见历史）；RPC `voice-control:_:log` 快照、`voice-control:_:log:clear` 清空。纯内存，重启即丢。
- **prompt 模板可配置（v0.50）**：系统模板与摘要模板从 config-store `plugins.voice-control` 逐调用重读（同 llm 插件 active 的即时生效契约），未配置/空值回退内置默认。占位符：系统模板 `{{catalog_section}}`（注入合并目录 JSON 或空目录说明），摘要模板 `{{summary}}`/`{{turns}}`。`voice-control:_:prompt {text?}` 为 debug pane 提供：生效模板、内置默认、当前会话摘要与历史、以及用示例转写实时渲染的完整 messages 列表。
- **effects 两类**：通用 `open_instance {pane_type, instance_id}`（voice-control 前端直接 `layout.openInstance`，chat 后端同步置 active 并发 `chat:_:active`）；插件私有（如 `start_dictation`）经 **retained mailbox** `voice-fx:<plugin>:<instance_id>` 交接目标 pane——带 nonce 时间戳去重 + 30s 过期（pane 刚挂载时 subscribe RPC 未就绪会丢普通 publish，mailbox 重放兜底；过期丢弃防页面重载重放旧帧）。pane 完成口述后 publish `voice-control:_:resume {type:"dictation_done"}`，loop 恢复听命令；60s 无应答兜底恢复。
- **chat 侧（v0.47 机制保留下沉）**：`chat:_:voice:invoke` 执行五动作（`open-chat:<id>` / `action:read-latest` / `action:dictate` / `action:stop` / `action:status`）；朗读改写（speakable）留在 chat 内部——read-latest 取最新 assistant 消息过 LLM 改写成口语摘要（去 hash/路径/代码块，≤3 句），失败回退去代码围栏 + 截断 200 字；口述 + 确认回路（TTS 念「说发送确认，说取消作废」，下一句本地匹配，3 次未匹配保留草稿退出）留在 chat pane 内。
- **入口**：Dock-foot 新增**全局动作扩展点** `createDockActions`（icon/title/active 为响应式 getter，任意插件可注册，渲染在 pin/设置按钮旁）；voice-control 的耳机 toggle 是第一个用户。composer 耳机按钮删除；Ctrl+Shift+M 提升为全局快捷键。voice-control 另挂 singleton Dock pane（v0.50）：日志 / Prompt / 功能目录三 tab 的调试面板。
- **主动播报（基本版）**：voice-control 订阅 `chat:_:turn`（全局 turn 生命周期 feed），loop 开启时 turn 完成自动念该聊天最新回复摘要（复用 chat 的 read-latest + speakable）；`stop_reason=error` 直接念「XX 的任务执行失败」（失败 turn 无新回复可读，不读陈旧消息），cancelled 静默。开关 = 前端 toggle 同步 `voice-control:_:enable {enabled}`；播报经 `voice-control:_:announce` 推前端，播放时暂停麦克风、播完恢复，防回声。
- **端点检测不变**（v0.47 结论保留）：不用带内关键词（无法区分命令与内容）；客户端静音端点检测（AnalyserNode RMS + 自适应阈值）：命令/确认 2s、口述 4s 尾静音自动结束；关键词只出现在确认回路（整句即答案，无歧义）与退出词（命令相整句本地匹配）。TTS = 浏览器 SpeechSynthesis 单槽队列，TTS 播完才开麦。命令/确认录音跳过 LLM refine（更快、保关键词），口述沿用用户 refine 设置。
...
- **v0.78**（2026-09-17）：**归档 = 合并进「归档」线 + 分支条计数徽章与「已归档」区移除**——①`branches:archive` 语义重定义为单步合并：源分支图 graft 到 chat 的终端归档线（确定性 id `archive:<chatID>`、名「归档」，首次归档时在合并事务内诞生，合并失败不留空线），被归档内容由此留在一条真实可查看、可派发、可再分叉的线上（从被归档 turn 分叉沿 `merged_into_branch_id` 链归属归档线）；归档线是终态汇点——拒绝作为 merge/archive 源、拒绝删除（mergeLines 源校验 + `branches:delete` 守卫）。②归档线自身是普通 open 分支（分支条正常 tab、可正常派发），「已归档」按钮与展开列表从分支条移除——查看归档内容 = 选中「归档」tab；v0.66 的 archive-only 搁置语义不再产生（旧数据行仍按只读处置）。③分支 tab 计数徽章移除：`chats:list` 的 `include_counts`/`line_message_counts`、批次落定后的 `chat:_:branch` phase `head` 帧与前端 `lineCounts` 一并删除（徽章是其唯一消费者）。
- **v0.77**（2026-09-17）：**引用按钮收进信息条 + 合并线 turn 可再分叉**——①chat 引用按钮从逐文本段 hover 浮钮收进消息盒顶信息条（与 fork 按钮并列，每盒一枚）：引用目标 = 盒内最后一条文本消息（user 盒即其唯一消息，role 盒为 turn 的最终文本），pending/发送中/发送失败盒无持久化消息、不显示；逐段 hover 机制与样式删除。②`branches:create` `from_turn_id` 放宽：fork turn 落在**已合并**线上时沿 `merged_into_branch_id` 链解析到存活线（16 跳上限防环；链终点为主线则 `parent_branch_id=""`）——其内容已并入合并目标历史，分叉 = 从目标线该点带出共享历史；**archive-only（搁置）线保持只读**，其 turn 不可分叉。前端 canFork 同口径：按 `merged_at` 区分合并线与搁置线。
...
- **v0.63**（2026-09-05）：**命名分支 + 合并回主线**——分支独立持久化（`branches` 表 + `turns.branch_id` 盖章），dispatch 新增 `branch_id`（续接/首轮 fresh），匿名 parallel 自动建分支；新增 `branches:create/patch/delete/merge/merge-confirm` 与 `chat:_:branch` feed；合并 = LLM 可编辑摘要（含分支间冲突节）→ 确认送入主线 + 归档（记录 cutoff turn 与 merge_message_id）；前端分支条（主线/分支/全部/已归档/＋/合并）取代 lane 条。
- **v0.62**（2026-09-03）：**chat 记忆写入（retain）**——每条可见消息异步 retain 到 Chat 级 Hindsight bank（用户消息在 dispatch 时、assistant 消息在 turn 完成时），`async:false` 同步抽取 + 真实 RFC3339 时间戳 + provider/role/message id 元数据；首次 retain 前 PATCH bank 关 auto-consolidation（单 worker daemon 防卡死）；失败仅记日志。配置 `hindsight.retain_enabled`（默认 true）/`retain_timeout_seconds`（默认 30）。
- **v0.61**（2026-09-03）：**排队消息可取消/可原位编辑**——新增 `chat:_:queued-cancel`（摘队列条目 + 删 user 消息行 + 墓碑帧）与 `chat:_:queued-update`（原位改文本、位置不变），已开始执行回 `not_queued`；`chats:list` 新增 `queued_messages` 快照，新增 `chat:_:queue` 全量 feed；前端 query box 顶条加 queued chip（两次点击取消）+ 铅笔编辑（载入 composer，发送即原位更新）。
- **v0.60**（2026-09-02）：**codex model/list 协议适配 + 模型手动输入**——codex ≥0.151 的 `model/list` 结果字段 `models` 改名 `data`，解析只认新键（不向后兼容，协议漂移以 catalog refresh 错误形式暴露，不再静默吃空表）；chat-manager 的 Model 选择器支持手动输入（RoutesPanel 的 TextSelect 新增 `allow-custom` 自由文本行，RolesPanel 候选构造器改 input+datalist），catalog 发现失败时仍可手填模型名。
- **v0.53**（2026-08-23）：**LLM HTTP 可观测性 + 容器接入**——HTTP 配置增 expose（loopback/LAN+container）；每个 completion 记录 request ID、原始/实际上游 body、完整状态与有界返回，Hindsight/infod/gaokao/voice-service 统一改走 Viewer 18731。
- **v0.52**（2026-08-23）：**LLM 本机统一 HTTP 接口**——新增 `plugins.llm.http` 热配置、`llm:_:http:configure/status` 与 loopback OpenAI-compatible `/v1/chat/completions|models` facade；HTTP 调用强制使用 active model，让 Viewer 和同机外部应用共享一次模型切换。
- **v0.51**（2026-08-22）：**语音对话历史协议化**——assistant 历史轮次改存规范化 `{"say","entry_id"}` JSON（修复纯文本历史带偏模型、第二轮起不再输出 JSON 导致条目不派发的问题）；摘要压缩对 assistant 轮次解包取朗读文本。
- **v0.50**（2026-08-22）：**语音控制可观测性 + prompt 模板可配置**——交互日志（100 条内存 ring，retained mailbox `voice-control:_:log` 全量镜像 + 快照/清空 RPC，command RPC 新增 `phase` 字段）；系统/摘要 prompt 模板存 config-store `plugins.voice-control` 逐调用重读（空值回退内置默认），`voice-control:_:prompt` 返回模板/默认/会话状态/渲染预览；voice-control 前端新增 singleton debug pane（日志/Prompt/功能目录三 tab）。
...
- **v0.59**（2026-08-27）：chat 折叠口径放宽（A.7）+ 浮动放回不再顶替（§8.7）：第一级 `Tools ×N` 分组计入 thinking/error 等全部小灰行活动，仅正文文本打断；`dockFloating` 平铺模式叶端追加新 tile（末位空 tile 直接填入），自由模式替换 active pane，不再走 `openInstance` 顶替现有内容。
- **v0.58**（2026-08-27）：输入会话从 pane 组件生命周期提升为全局持久化注册表：草稿/目标/角色/一次性选项/pin 偏好落 localStorage，插件级 send adapter 允许 pane 关闭后继续发送；voice 录音不随组件卸载取消，抬头栏可 stop/send，send 严格等待 final/refine；Composer 支持 pin 与失焦缩成右下角 overlay。
- **v0.57**（2026-08-27）：工具活动两层折叠与语义化详情（A.7）：连续 ≥3 条工具项折成稳定 `Tools ×N` 第一层，逐项为第二层；摘要/详情分别限长并按命令、路径、输入、输出、diff 等字段呈现；ACP 嵌套 content 正规化，Codex command lifecycle 按 item id 聚合为单 block。
...
- **v0.46**（2026-08-21）：**dispatch router 走共享 LLM helper + 失败落日志**——`routeWithLLM` 改调 `chatCompletion`（endpoint 归一化补 `/chat/completions`），修复配置为 `…/v1` 形式时 router 裸 POST 根路径必 404 的「不指定模型必失败」；router 失败写 slog.Warn 到 viewerd.log。
- **v0.47**（2026-08-21）：**语音命令 slice 1（§8.11 Voice Actions）**——`chat:_:voice:command` RPC（LLM 意图解析 → 两级条目 open_instance/action → say 口语行 + effects），read-latest 可朗读摘要改写；前端语音命令按钮（Ctrl+Shift+M）、静音端点检测（`voice/silence.ts`）、SpeechSynthesis 朗读（`voice/speak.ts`）、口述确认回路（发送/取消本地匹配）；`voice:_:start` 的 `llm_refine` 支持按调用覆盖。
- **v0.45**（2026-08-21）：**chat LLM 配置库 + dispatch 超时修复**——模型 tab 改 master-detail（`llm_profiles` 配置库 + `llm` active + `timeout_seconds`）；共享 `http.Client` 30s 硬顶移除，router/summary/hindsight 各自 ctx 限时。
- **v0.44**（2026-08-21）：**计划重启可取消 + 前端状态可见 + Dock flat 化**——①`DELETE /api/admin/schedule-restart`：armed/building/failed 归位 `none`（幂等；构建中取消则构建跑完但不武装，building→armed/failed 转换改 CAS）；agent HTTP 闭环 POST/GET/DELETE。②Settings「空闲时重启」改 toggle 实时反映 GET 状态（5s 轮询），building/armed 点击即取消，状态行常显四态。③Dock 底部计划重启指示器（15s 轮询，warning/danger 色图标，点击开 Settings）。④Dock 侧边栏阴影移除（flat 风格）。
- **v0.43**（2026-08-20）：**计划重启带构建 + shell 浮动层**——①`schedule-restart` 取代 v0.42 纯武装语义：POST 先跑后台 release build（共用 buildMu 守卫，冲突 409），成功才武装等空闲看门狗，失败落 `failed` 不武装；新增 `GET /api/admin/schedule-restart`（none/building/armed/failed），前端失败即停等并提示日志，`failed` 可重新 POST。②浮动层：pane 抬头浮动按钮将内容提出 split 树（`floatPane`），浮动面板支持标题栏拖动、右下角拉伸、点击置顶（z 序）、放回布局（`dockFloating` 走 `openInstance`）、关闭；`openInstance`/`isUidOpen` 覆盖浮动层（Dock 点击已浮动实例 = 置顶）。
...
- slots：`send-message`（v0.37 起支持一次性 `force_new_session: true`——跳过 runtime 复用与 `role_sessions` 恢复，所有选中 role 强制新建 agent session；v0.60 起支持 `continue_turn_id` lane 续接——跳过路由、resume 指定 turn 的 session，lane 级 busy/queue，详见 v0.60 条目；v0.63 起支持 `branch_id` 分支路由 + `parallel_dispatch` 自动建分支，详见 v0.63 条目）、`stop`、`queued-cancel`/`queued-update`（v0.61 排队消息管理，详见 v0.61 条目）、`branches:create`/`branches:patch`/`branches:delete`/`branches:merge`/`branches:merge-confirm`/`branches:archive`（v0.63 命名分支与合并、v0.66 手动归档，详见 v0.63/v0.66 条目）、`blocks:list`（`{chat_id, after?, before?}` → `{blocks[]}` 按 `occurred_at` 排序，`after`/`before` 限定时间窗 [after, before) ms，缺省全量；**v0.31 起时间线按已加载消息跨度分窗取块**）；`chats:list` include_messages 分页（v0.31/v0.32）：`{chat_id, include_messages: true, before?, before_id?, after?, after_id?, limit?}` → `{chats, messages[], has_more}`，游标 = 复合 `(created_at, id)`：`before` 严格小于（v0.31 上翻页），`after` **含边界**（v0.32 增量拉取：重拉边界行本身 + 全部更新行，升序返回，`has_more` 表示还有更新行——前端循环取完；边界行重拉以便用最终文本替换可能仍在流式的缓存副本），消息升序返回；emits：`chat:{id}:turn-completed`、`chat:{id}:message`、`chat:{id}:block`（块落库即推，v0.30）、`chat:_:active`（mailbox，CWD 联动的 source）。
- **消息渲染（v0.30，v0.57 工具层升级，v0.59 折叠口径）**：一 turn 一盒（user 消息独立盒；role turn 一盒），盒顶 info 条 = 角色图标+名 + 时间（+活跃态 spinner）；盒内 segments 严格按 `occurred_at` 交错——`messages` 文本段走 markdown 渲染；连续 ≥3 条小灰行活动（tool_call/tool_result/file_change/command/thinking/error）收成 `Tools ×N` 第一层（仅正文文本打断），展开后每项仍是可展开第二层。单项外层以图标+语义类型+命令/路径摘要+状态+时间显示，摘要 220 字符封顶；内层按 Command/Path/Locations/Input/Arguments/Output/Result/Changes 分区，总展示预算 9000 字符并显式标注省略，不直接渲染裸 payload JSON。markdown 管线 = markdown-it + texmath(KaTeX) + hljs（行号）+ mermaid（对齐生产版 `markdownRender.ts`）；样式 = `--markdown-*`/`--syntax-*` CSS 变量主题（亮/暗内置默认随 app theme，自定义存 localStorage `viewer.markdownTheme.v1`）。
