  ...
  <header class="pane-titlebar" :class="{ active: isActive }" @pointerdown="activate">
    ...
    <div v-if="toolbar?.actions?.length" class="pane-titlebar-actions" aria-label="Pane-specific actions">
      <button
        ...
        :key="action.id"
        class="btn btn-outline-secondary icon-button pane-titlebar-action"
    ...
    <div class="pane-titlebar-actions pane-layout-actions" aria-label="Pane layout actions">
      <button class="btn btn-outline-secondary icon-button pane-titlebar-action" type="button" title="Refresh pane" @pointerdown.stop @click.stop="refreshPane">
        <i class="bi bi-arrow-clockwise"></i>
      </button>
      <button v-if="canGoBack" class="btn btn-outline-secondary icon-button pane-titlebar-action" type="button" title="Go back" @pointerdown.stop @click.stop="goBack">
        <i class="bi bi-arrow-left"></i>
      </button>
      <button class="btn btn-outline-secondary icon-button pane-titlebar-action" type="button" title="Split pane right" @pointerdown.stop @click.stop="splitPane('vertical')">
        <i class="bi bi-layout-split"></i>
      </button>
      <button class="btn btn-outline-secondary icon-button pane-titlebar-action" type="button" title="Split pane down" @pointerdown.stop @click.stop="splitPane('horizontal')">
        <i class="bi bi-view-stacked"></i>
      </button>
      <button class="btn btn-outline-secondary icon-button pane-titlebar-action danger" type="button" :title="hasContent ? 'Clear pane content' : 'Close empty pane'" @pointerdown.stop @click.stop="closePane">
...
.pane-titlebar {
  ...
  min-width: 0;
  overflow: hidden;
  padding: 2px var(--pane-content-inline-padding);
}
