...
safe. Worker/lease/checkpoint/health/raw-event tables are intentionally ignored.
"""
...
def migrate(source: Path, target: Path) -> dict[str, int]:
    ...
    counts = {"chats": 0, "skipped_chats": 0, "messages": 0, "turn_summaries": 0, "role_sessions": 0}
...
CREATE TABLE super_workspace_messages(id TEXT,turn_id TEXT,conversation_id TEXT,role_id TEXT,sender_role_id TEXT,role TEXT,text TEXT,query TEXT,occurred_at REAL,received_at REAL);
...
""")
    db.execute("INSERT INTO super_workspace_chats VALUES(?,?,?,?,?,?,?,?,?,?,?)",("chat-1","default","dailing","Fixture","group","/tmp","common",'["role-1"]','{"role-1":"route"}',1.0,2.0));db.execute("INSERT INTO super_workspace_chat_pins VALUES(?,?,?,?,?)",("dailing","default","chat-1",1.0,2.0));db.execute("INSERT INTO super_workspace_roles VALUES(?,?)",("role-1","Builder"));db.execute("INSERT INTO super_workspace_messages VALUES(?,?,?,?,?,?,?,?,?,?)",("user-1","dispatch-1","chat-1",None,None,"user","","hello",3.0,3.0));db.execute("INSERT INTO super_workspace_messages VALUES(?,?,?,?,?,?,?,?,?,?)",("answer-1","turn-1","chat-1","role-1",None,"assistant","done",None,4.0,4.0));db.execute("INSERT INTO super_workspace_turn_summaries VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",("turn-1","chat-1","role-1","Builder","hermes","completed","summary","model","profile",2,9,12,"",4.0,5.0));db.execute("INSERT INTO super_workspace_chat_role_sessions VALUES(?,?,?,?,?,?,?,?,?)",("chat-1","role-1","hermes","provider-session","session-ref","viewer-session","/tmp","model",6.0));db.execute("INSERT INTO super_workspace_driver_runs VALUES(?)",("ignored",));db.commit();db.close()
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="viewer-chat-migration-test-") as temporary:
        ...
        assert first=={"chats":1,"skipped_chats":0,"messages":2,"turn_summaries":1,"role_sessions":1},first
        assert second["chats"]==0 and second["skipped_chats"]==1,second
