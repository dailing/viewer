...
import type { AgentCatalog, Role, RoutingConfig, RoutingPolicy } from "../chat/types";
...
function edit(role: Role): void {
  Object.assign(form, role, { context_recycle_percent: role.context_recycle_percent?.toString() ?? "", context_recycle_tokens: role.context_recycle_tokens?.toString() ?? "" });
}
...
async function save(): Promise<void> {
  try {
    const payload = { ...form, context_recycle_percent: form.context_recycle_percent === "" ? null : Number(form.context_recycle_percent), context_recycle_tokens: form.context_recycle_tokens === "" ? null : Number(form.context_recycle_tokens) };
...
async function addTargetToPolicy(): Promise<void> {
  ...
  if (policy === undefined) {
    policy = { id: `policy-${uid}`, name: `${form.name || "Role"} policy`, enabled: true, auto_failover: false, max_attempts: 1, candidates: [] };
  ...
  policy.candidates.push({ id: `candidate-${uid}`, name: `${target.agent}/${target.provider}`, agent_id: target.agent, provider_id: target.provider, model_id: target.model, enabled: true, parameters: {} });
...
async function remove(id: string): Promise<void> {
  ...
  try { await ctx.bus.request("chat:_:roles:delete", { id }); await load(); } catch (cause) { error.value = errorText(cause); }
}
...
<template>
  <section class="h-100 overflow-auto p-2">
    ...
    <div class="row g-1">
      <div class="col-md-6"><input v-model="form.name" class="form-control form-control-sm" placeholder="Name"></div>
      <div class="col-md-6"><select v-model="form.routing_policy_id" class="form-select form-select-sm"><option value="">No policy</option><option v-for="policy in routing.routing_policies" :key="policy.id" :value="policy.id">{{ policy.name }}</option></select></div>
      <div class="col-12"><input v-model="form.description" class="form-control form-control-sm" placeholder="Router description"></div>
      <div class="col-12"><textarea v-model="form.prompt" class="form-control form-control-sm" rows="4" placeholder="Run prompt"></textarea></div>
      <div class="col-md-6"><input v-model="form.cwd" class="form-control form-control-sm" placeholder="Role cwd"></div>
      <div class="col-md-6"><select v-model="form.session_policy" class="form-select form-select-sm"><option value="reuse">reuse</option><option value="new_each_run">new_each_run</option></select></div>
      <div class="col-md-6"><input v-model="form.context_recycle_percent" type="number" class="form-control form-control-sm" placeholder="Context recycle %"></div>
      <div class="col-md-6"><input v-model="form.context_recycle_tokens" type="number" class="form-control form-control-sm" placeholder="Context recycle tokens"></div>
    </div>
    ...
    <div class="d-flex gap-1 my-3"><button class="btn btn-sm btn-primary" @click="save">{{ form.id ? "Save" : "Create" }}</button><button class="btn btn-sm btn-outline-secondary" @click="reset">Clear</button></div>
    <div v-for="role in roles" :key="role.id" class="d-flex align-items-start gap-1 border-top py-1"><button class="btn btn-sm text-start flex-grow-1" @click="edit(role)"><strong>{{ role.name }}</strong><small class="d-block text-secondary">{{ role.description }} · {{ role.routing_policy_id || "no policy" }}</small></button><button class="btn btn-sm text-danger" @click="remove(role.id)"><i class="bi-trash"></i></button></div>
  </section>
</template>
