...
export interface Role { id: string; name: string; description: string; prompt: string; provider: "hermes"; cwd: string; model: string | null; routing_policy_id: string; capability_requirements: Record<string, unknown>; session_policy: string; context_recycle_percent: number | null; context_recycle_tokens: number | null; created_at: number; updated_at: number }
