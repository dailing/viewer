/**
 * Markdown rendering pipeline (ported from the original viewer's
 * markdownRender.ts, framework v0.30): markdown-it + KaTeX (texmath) +
 * highlight.js with line numbers + mermaid fences. Chat text segments render
 * through renderMarkdown(); local-file link/image rewriting is dropped (the
 * file-service raw URL flow is not part of the chat data plane).
 */
