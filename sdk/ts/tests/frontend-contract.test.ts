/**
 * Executable contracts for the bus calls made by frontend's terminal and
...
const VIEWERD_BIN = process.env.VIEWERD_BIN
  ...
  : path.join(REPO_DIR, "bin", "viewerd");
...
function startViewerd(gatewayPort: number, kernelPort: number, dataDir: string, plugins = "bus-inspector,terminal,file-service"): ChildProcess {
  return spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        VIEWER_HERMES_COMMAND: path.join(REPO_DIR, "scripts", "mock_acp_agent.py"),
        VIEWER_HERMES_PROFILE: "frontend-contract",
        VIEWER_CODEX_APP_SERVER_COMMAND: path.join(REPO_DIR, "scripts", "mock_codex_server.py"),
      },
    },
  );
}
...
describe.sequential("frontend pane contracts against viewerd", () => {
  let gatewayPort = 0;
  let kernelPort = 0;
  let dataDir = "";
  let filesFixture = "";
  ...
  it("lists lazy nested file trees with production fields, sorting, hidden filtering, symlinks, and errors", async () => {
    ...
    expect(current.path).toBe(REPO_DIR);
  ...
  it("drives chat roles/chats/active mailbox, dual-role relay, busy, and router errors", async () => {
    ...
    const chat = (await client.request("chat:_:chats:create", { name: "Contract", root: REPO_DIR, member_role_ids: [first.id, second.id] })) as { id: string };
